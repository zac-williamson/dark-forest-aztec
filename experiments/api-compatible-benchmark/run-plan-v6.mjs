...
export const baseline='baseline-v2',candidate=process.env.API_BENCH_CANDIDATE??'candidate-v6';
assert(/^candidate-v6(?:-[a-z0-9_-]+)?$/.test(candidate),'Use a fresh V6 label; never overwrite V4/V5 evidence');
const originalArtifacts='/tmp/df-fee-tools/artifacts/baseline';
const candidateArtifacts=path.resolve(process.env.API_BENCH_ARTIFACTS??'/tmp/df-api-compatible-v6-native');
const findPair=process.env.API_BENCH_FIND_PAIR??'v6';assert(/^v6(?:_[a-z0-9_]+)?$/.test(findPair),'Find needs a new V6 pair label');
const findCase=`find_${findPair}`;
const common={BENCH_WALLET_DIRECTORY:process.env.API_BENCH_WALLET_DIRECTORY??'/tmp/df-api-compatible-wallet52-v6',BENCH_BASELINE_VARIANT:baseline,
  BENCH_FIND_PAIR:findPair,BENCH_ASSERT_BASELINE_FIXTURE:'1',AZTEC_NODE_URL:process.env.AZTEC_NODE_URL??'http://127.0.0.1:8097',NODE_BACKEND:'js',LOG_LEVEL:'error',RAYON_NUM_THREADS:'2',HARDWARE_CONCURRENCY:'2'};
...
const action=(id,variant,env={})=>({id,program:'run.mjs',args:[variant,variant===baseline?originalArtifacts:candidateArtifacts],env:{...common,...env,...(variant===candidate?{BENCH_FINAL_TX_GATE:'1'}:{})}});
...
export const phases={
  first4:[setup('setup.mjs'),setup('configure.mjs'),
    ...
    ...paired('fresh-find',{BENCH_METHODS:'find_artifact',BENCH_CASES:findCase}),
    action('refresh-and-move',candidate,{BENCH_METHODS:'refresh_planet,move'})],
  remaining28:[action('remaining-private',candidate,{BENCH_METHODS:remainingPrivate.join(',')}),
    action('public-admin-and-vault',candidate,{BENCH_PUBLIC:'1'}),
    // Baseline Move20 is cached at ID3; this must be candidate's second allocation.
    action('maximum-due-arrivals',candidate,{BENCH_METHODS:'refresh_planet,move',BENCH_CASES:'20'})],
  edges:[action('initialization-edges',candidate,{BENCH_METHODS:'initialize_player',BENCH_CASES:'init_unused_padding,init_existing_planet'}),
    action('refresh-edges',candidate,{BENCH_METHODS:'refresh_planet',BENCH_CASES:'refresh_unused_padding,1'})],
  extended:[action('retained-move-branches',candidate,{BENCH_METHODS:'move',BENCH_CASES:moveExtras.join(',')}),
    action('refresh-artifact-arrival',candidate,{BENCH_METHODS:'refresh_planet',BENCH_CASES:'artifact_arrival'})],
  sponsored:[action('sponsored-player-actions',candidate,{BENCH_METHODS:'refresh_planet,move',BENCH_SPONSORED:'1'})],

};

const signature=(method,caseId='ordinary',paymentMode='account')=>({method,caseId,paymentMode});
export const initialCoverage=[signature('initialize_player'),signature('find_artifact',findCase),signature('refresh_planet'),signature('move')];
...
export const plan={baseline,candidate,originalArtifacts,candidateArtifacts,
  ...
  freshBaselineRequired:[signature('find_artifact',findCase)],cachedBaselineRows:44,
  transactionProofsEnabled:false,diagnosticProfilingEnabled:false};
  ...
  const rows=expected.map(key=>{
    ...
    const actor=row=>{
      ...
      if(row.variant===candidate){
        assert.equal(row.fixtureReplay?.exactOriginalInputAndSeeds,true,'V6 requires exact cached original fixture replay');
        assert.equal(row.simulatedOriginalStateEquivalent,true,'V6 requires whole-transaction effect equivalence before broadcast');
        assert.equal(row.finalTransactionGate?.feeEnforcementEnabled,true,'V6 must check the exact final transaction with fee enforcement');
        assert.equal(row.finalTransactionGate?.standalonePrivateSimulation,false,'V6 avoids an additional standalone private simulation');
        assert.equal(row.finalTransactionGate?.transactionHash,row.receipt.txHash,'V6 must mine the identical gated transaction');
        assert.deepEqual(fixture.replay,row.fixtureReplay,'Saved input and receipt must retain identical replay provenance');
        const originalBytes=fs.readFileSync(new URL(`.state/${row.fixtureReplay.originalFixture}`,import.meta.url));
        assert.equal(createHash('sha256').update(originalBytes).digest('hex'),row.fixtureReplay.originalFixtureSHA256,'Original paired fixture changed since the candidate action');
      }
      return actual;
    };
...
async function main(){
  ...
  const selected=process.argv[3];assert(['first4','all','remaining28','edges','extended','sponsored'].includes(selected),'Select an explicit execution phase');
  ...
  const saved=records(),expected=[...ordinaryCoverage,...edgeCoverage,...extensionCoverage,...sponsorCoverage];
  const snapshot=path.join(directory,`results/${candidate}-build-provenance.json`);
  ...
  for(const variant of [baseline,candidate]){
    ...
    for(const key of expected){
      ...
      if(variant===baseline&&key.method!=='find_artifact')assert(row,'All44 cached original comparisons are required before candidate setup');
  ...
  await execute({id:'verify-cached-original-receipts',program:'verify-cached-baseline.mjs',args:[baseline,candidate],env:{...common}});
  ...
  if(selected==='all'){
    ...
    fs.writeFileSync(path.join(directory,`results/coverage-${candidate}.json`),JSON.stringify({candidate,verifiedPairs:verified.length,rows:verified},null,2)+'\n');
  }else if(selected!=='first4'){
    inspectCoverage(records(),initialCoverage,{requireNoFeeIncrease:true});
    const prerequisites={edges:ordinaryCoverage,extended:[...ordinaryCoverage,...edgeCoverage],sponsored:[...ordinaryCoverage,...edgeCoverage,...extensionCoverage]};
