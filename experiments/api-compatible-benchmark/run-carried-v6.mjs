...
import {inspectCoverage} from './run-plan-v6.mjs';

const directory=fileURLToPath(new URL('./',import.meta.url)),repo=path.resolve(directory,'../..');
const baseline='baseline-v2',candidate='candidate-v6';
const folders={[baseline]:'/tmp/df-fee-tools/artifacts/baseline',[candidate]:'/tmp/df-api-compatible-v6-native'};
export const cases=[
...
function validateArtifacts(){
 ...
 for(const item of manifest.artifacts)assert.equal(createHash('sha256').update(fs.readFileSync(path.join(folders[candidate],item.file))).digest('hex'),item.sha256,`Frozen V6 artifact changed: ${item.file}`);
...
async function main(){
 if(process.argv[2]!=='--execute'){console.log(JSON.stringify({steps,proofsEnabled:false,profilingEnabled:false,threads:2,coverageOutput:'coverage-carried-candidate-v6.json'},null,2));return;}
 validateArtifacts();
 const frozen=JSON.parse(fs.readFileSync(path.join(directory,'results/coverage-candidate-v6.json')));
 ...
 for(const step of steps){
  ...
  Object.assign(env,{BENCH_METHODS:step.method,BENCH_CASES:step.caseId,BENCH_BASELINE_VARIANT:baseline,BENCH_ASSERT_BASELINE_FIXTURE:'1',
   BENCH_WALLET_DIRECTORY:'/tmp/df-api-compatible-wallet52-v6',BENCH_FIND_PAIR:'v6',AZTEC_NODE_URL:'http://127.0.0.1:8097',
   NODE_BACKEND:'js',LOG_LEVEL:'error',RAYON_NUM_THREADS:'2',HARDWARE_CONCURRENCY:'2'});
  if(step.variant===candidate)env.BENCH_FINAL_TX_GATE='1';
 ...
 fs.writeFileSync(path.join(directory,'results/coverage-carried-candidate-v6.json'),JSON.stringify(report,null,2)+'\n');
