...
class LocalRootArchitectureTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        shutil.copytree(ROOT/'contracts',cls.root/'contracts',ignore=shutil.ignore_patterns('target','.git'))
    ...
    def test_every_root_writer_keeps_authorization_before_write_and_event(self):
        for slug in generate_facades.KINDS:
            ...
            self.assertLess(setter.index('local_roots.at('),setter.index('self.emit('))
            if slug=='arrival':
    ...
    def test_original_nonarrival_bodies_only_rename_root_addressing(self):
        for slug in generate_facades.KINDS:
            ...
            for function in check.parse_source(slug,original)['functions']:
                ...
                self.assertEqual(check.norm(body(current,function['name'])),check.norm(expected),(slug,function['name']))
