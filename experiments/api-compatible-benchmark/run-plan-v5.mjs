...
export const baseline='baseline-v2',candidate=process.env.API_BENCH_CANDIDATE??'candidate-v5';
assert(/^candidate-v5(?:-[a-z0-9_-]+)?$/.test(candidate),'Use a fresh V5 label; never overwrite V4 evidence');
const originalArtifacts='/tmp/df-fee-tools/artifacts/baseline';
const candidateArtifacts=path.resolve(process.env.API_BENCH_ARTIFACTS??'/tmp/df-api-compatible-v5-native');
const findPair=process.env.API_BENCH_FIND_PAIR??'v5';assert(/^v5(?:_[a-z0-9_]+)?$/.test(findPair),'Find needs a new V5 pair label');
const findCase=`find_${findPair}`;
const common={BENCH_WALLET_DIRECTORY:process.env.API_BENCH_WALLET_DIRECTORY??'/tmp/df-api-compatible-wallet52-v5',BENCH_BASELINE_VARIANT:baseline,
  ...
  const rows=expected.map(key=>{
    ...
    const actor=row=>{
      ...
      if(row.variant===candidate){
        assert.equal(row.fixtureReplay?.exactOriginalInputAndSeeds,true,'V5 requires exact cached original fixture replay');
        assert.equal(row.simulatedOriginalStateEquivalent,true,'V5 requires whole-transaction effect equivalence before broadcast');
