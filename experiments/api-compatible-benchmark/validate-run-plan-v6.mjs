...
import {plan,phases,ordinaryCoverage,edgeCoverage,extensionCoverage,sponsorCoverage,validateMoveOrder} from './run-plan-v6.mjs';
...
assert.equal(originalSteps[0].env.BENCH_FINAL_TX_GATE,undefined);
...
for(const step of actions.filter(s=>s.args[0]!==plan.baseline)){
 ...
 assert.equal(step.env.BENCH_FINAL_TX_GATE,'1');
}
